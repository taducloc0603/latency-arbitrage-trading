...
- Khi du samples (post-warmup):
  ...
  - Floor `-50 / +35` la san: dynamic chi duoc dung khi extreme hon. Trong market calm, std nho khien dynamic lon hon san; khi do bot dung san de tranh trade tren noise.
- Nguong dong co dinh:
  - `CloseBuyRevert = 0`
  - `CloseSellRevert = 0`
- Feed stale:
...
- `FixedOpenBuyFallback` / `FixedOpenSellFallback` (`-50` / `35`): san floor cho threshold mo lenh. Cung la nguong dung trong warmup.
