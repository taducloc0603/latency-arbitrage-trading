...
public sealed class CsvLoggerTests
{
    ...
    public void LogSnapshot_WritesRow()
    {
        InTempDir(logsDir =>
        {
            ...
            using (var logger = new CsvLogger(logsDir))
            {
                logger.LogSnapshot(1234, a, b, gapBuy: -22, gapSell: 22,
            ...
            Assert.Equal("timestamp,aBid,aAsk,bBid,bAsk,spreadA,spreadB,gapBuy,gapSell,winState,winDurMs,winMin,winMax,winN", lines[0]);
